...
import androidnfc.movieapp.models.SerializableShowList;
	...
	private void openMovieDetails(SearchResultMovie movie) {
		...
		if (movie.getImdbId() == null || movie.getImdbId().isEmpty()) {
			...
			if (searched.size() == 0) {
				...
				SerializableShowList shows = movie.getShows();
				int finnkinoId = movie.getFinnkinoId();
				movie = searched.get(0);
				movie.setFinnkinoId(finnkinoId);
		...
		m.setShowListString(m.getShows().get(0).getShowStart());
